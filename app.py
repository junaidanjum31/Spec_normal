...
st.markdown("**Multi-file Stacking with Reference Normalization**")
...
if uploaded_files:
    for file in uploaded_files:
        try:
            ...
            df = df[[numeric_cols[0], numeric_cols[1]]].dropna().copy()
            df.columns = ['x', 'y']
...
if data_dict and normalization_mode == "Stack & Normalize Together" and len(data_dict) > 1:
    ref_file = st.selectbox(
        "🔑 Select Reference File (its max will normalize all others)", 
        list(data_dict.keys()), index=0
    )
...
if data_dict:
    ...
    st.info("👆 Upload multiple files to test stacking & reference normalization")